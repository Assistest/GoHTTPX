...
>
> 适用范围：当前目录 `gohttpx/`。父目录中的旧 Go 项目不属于 GoHTTPX。
...
GoHTTPX 的源码边界只有当前 `gohttpx/` 目录。

父目录中的这些内容属于旧项目，不得复制、引用、迁移或顺手重构：
...
当前 Git worktree 会显示完整父仓库，这是 Git worktree 的正常行为，不代表父目录文件属于新项目。构建和测试必须在 `gohttpx/` 内执行；该目录有独立的 `go.mod`。

后续提交 GoHTTPX 时：

1. 默认只修改 `gohttpx/` 及直接描述 GoHTTPX 的设计文档。
2. 不删除父项目文件，否则合并时会同时破坏旧项目。
3. 新项目需要独立发布时，只发布 `gohttpx/`，不要把父仓库一起打包。
